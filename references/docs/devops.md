...
* **绝对禁忌**：严禁引入任何角色战斗数值、技能机制、光锥/遗器配队、副本关卡打法等与战斗系统相关的干预信息。光锥故事文本与光锥卡面可作为剧情、角色记忆和视觉设定资料保留，但不得用于玩法建议。
...
2. **高价值筛选**：
   ...
   - 默认排除 `{{图标|...}}`、小尺寸 `[[file:...|64px]]` 图标、CSS 装饰图、元素/机制图标与光锥小图标。
3. **角色立绘补全**：
   ...
   - 光锥页大图通过 `光锥-立绘-{光锥名}.png` 推断；若 `|名称=` 为 `{{PAGENAME}}`，回退使用页面标题。只保留大图卡面，不保留同名 256px 图标。
4. **原图解析与去重**：
