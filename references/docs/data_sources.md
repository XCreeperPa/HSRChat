...
*   **BWiki 图片信源**：当前由 `scripts/sync_bwiki_images.py` 基于现有 `references/wiki/` 与角色页命名规则派生，不在 `config.json` 中默认启用，避免常规同步时意外下载大体积图片。图片文本描述索引位于 `references/bwiki_images/vision_index/assets.jsonl`，用于让非多模态模型读取已审核的图片元素描述。
...
*   **图片文本描述边界**：`references/bwiki_images/vision_index/assets.jsonl` 是审核通过的正式图片文本描述索引，应纳入版本控制；`references/bwiki_images/vision_jobs/` 是并发生成过程目录，不提交；`references/bwiki_images/vision_review/review_state.json` 是本地审核草稿，不提交。
*   **一键回滚（若发现异常）**：
    ```bash
