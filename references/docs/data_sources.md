...
*   **BWiki 图片信源**：当前由 `scripts/sync_bwiki_images.py` 基于现有 `references/wiki/` 与角色页命名规则派生，不在 `config.json` 中默认启用，避免常规同步时意外下载大体积图片。图片文本描述目录位于 `references/bwiki_images/vision_index/assets/`，结构与 `assets_webp/` 一致，用于让非多模态模型读取已审核的图片元素描述；`assets.jsonl` 保留为聚合兼容索引。
...
| `review_bwiki_vision_json.py` | BWiki 图片文本描述 | 启动本地审核 GUI，自动合入 LLM JSONL 输出，支持逐图审核、字段错误标注和导出审核通过描述。 |
| `bwiki_vision_merge_jobs.py` | BWiki 图片文本描述 | 合并并校验并发 worker 生成的图片描述 JSONL，检查漏图、重复路径和 JSON 格式。 |
