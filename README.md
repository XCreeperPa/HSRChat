...
- **问同人创作**：按 `core/policies/creative_writing.md` 判断官方贴合、兼容延展、原作分歧、AU 或私设，再选择合适的写作框架。
...
HSRChat 现在按“设定的使用方式”和“用户目标”分成四种模式，而不是按回答长短分：
...
| 创作 | 可写、贴合、边界清楚 | 作为创作约束和灵感材料 | “写一段列车组日常”“帮我检查这段是否 OOC” |
...
├── SKILL.md             # 兼容期 Skill 入口与运行模式行为规范
├── roleplay.md          # 扮演模式指南
├── config.json          # Wiki 与 B站分类配置
├── README.md            # 项目入口说明
├── core/                # 平台无关的运行策略、检索契约与数据 schema
│   ├── policies/        # 运行模式、创作、玩法过滤、信源优先级与扮演策略
│   ├── retrieval/       # 多跳检索与引用契约
