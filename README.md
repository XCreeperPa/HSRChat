...
HSRChat 是一个基于 **Gemini CLI Custom Agent Skill** 规范开发的项目，旨在为 Agent 提供深度交流《崩坏：星穹铁道》（Honkai: Star Rail）世界观、主支线剧情、以及角色设定所需的完整、权威的官方参考语料。

此 Skill 采用**渐进式披露（On-demand Activation）**设计。在交互时，唯有用户提起“星穹铁道”或“崩铁”等相关词汇时，才会触发并激活此 Skill，将对应的语料引入到 Agent 上下文中。
...
└── scripts/
    ...
    └── crawler.py        # 脚本 2：并发爬虫（支持测试模式与多余文件夹自动清理）
```
...
本项目采用**单 Git 仓库管理模式**，将所有的同步状态和 Wiki 文本全部纳入根目录 Git 中管理。在运行爬虫更新数据时，请严格遵守以下三步安全工作流：
